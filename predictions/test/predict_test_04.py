...
def predict(date_time, count):
    ...
    #data_from2012_to2015 = pd.read_csv(path + '/EURUSD_M1_2017_indicators_with_OHLC_rsi_ema_willr_window_10.csv')  # .iloc[:1000]
    data_from2012_to2015 = pd.read_csv(path + '/EURUSD_M1_2016_indicators_with_OHLC_rsi_ema_willr_window_10.csv')
    ...
    min_maxed_dataset = min_max(data_from2012_to2015, 'minmaxvalues_all_data_2016', column_list=column_list)
    ...
    #save_file = 'E:\\moodle\\Level04S02\\Teamfx\\weights_record\\T39-2018-06-23\\2017_epochs_1500_relu_next_3_close_price_for_close_ema_rsi.ckpt'
    save_file = 'E:\\moodle\\Level04S02\\Teamfx\\weights_record\\T40-2018-06-23\\2016_epochs_1500_relu_next_3_close_price_for_close_ema_rsi.ckpt'
    ...
    with sess.as_default():
        ...
        predicted_close = invert_min_max_for_one_feature(np.array(predicted_close), 'minmaxvalues_all_data_2016',
                                                         'next_close')
        ...
        predicted_next_next_close = invert_min_max_for_one_feature(np.array(normalized_next_next_close),
                                                                   'minmaxvalues_all_data_2016', 'next_next_close')
        ...
        predictions_list = invert_min_max_for_one_feature(np.array(predictions_next_close_label),
                                                          'minmaxvalues_all_data_2016', 'next_close')
        ...
        predictions_next_next_next_close_list = invert_min_max_for_one_feature(
            np.array(predictions_next_next_next_close_label), 'minmaxvalues_all_data_2016', 'next_next_next_close')
...
def predict_arima_ann(date_time, count):
    errors, error_next_next_close, error_next_next_next_close, predictions_from_ann, forwared_date_time, predicted_next_next_close, forward_next_next_date_time, predicted_next_next_next_close, forward_next_next_next_date_time, actual_values_forward, actual_close, actual_next_next_close, actual_next_next_next_close, backward_date_time = predict(
        date_time, count)
    ...
    final_results_next__next_next_close = np.array(predicted_next_next_next_close) + np.array(
        arima_output_for_next_next_next_close)
    ...
    f_n_n = [item for sublist in next_next_close_ann_arima for item in
             sublist]  # [x for x in next_next_close_ann_arima]
    next_next_next_close_ann_arima = final_results_next__next_next_close.values.tolist()
    f_n_n_n = [item for sublist in next_next_next_close_ann_arima for item in
               sublist]  # [x for x in next_next_next_close_ann_arima]
    ...
    labels_mix_final.append(f_n[0])
    labels_mix_final.append((f_n[1] + f_n_n[0]) / 2)
    labels_mix_final.append((f_n_n[1] + f_n_n_n[0]) / 3)
    # f_n[2]*0.4 +
