...
def predict(date_time, count):
    ...
    path = 'E:/moodle/Level04S02/Teamfx/preprocessing'
    column_list = ['open', 'high', 'low', 'close', 'will_r', 'ema', 'rsi', 'next_close']
    data_from2012_to2015 = pd.read_csv(path + '/EURUSD_M1_2012_indicators_with_OHLC_2018-06-18.csv')
    ...
    print(features)
    labels = min_maxed_dataset[['next_close']]
    ...
    with sess.as_default():
        ...
        for i in range(count):
                ...
                df = df.append({'close': predict_value[0].item()}, ignore_index=True)
                predicted_close.append(predict_value[0].item())
        ...
        predicted_close = invert_min_max_for_one_feature(np.array(predicted_close), 'minmaxvalues_all_data_2012', 'next_close')
        ...
        while j <= len(df_2):
            ...
            predictions_list.append(predict_value[0].item())
            j += 1
        ...
        predictions_list = invert_min_max_for_one_feature(np.array(predictions_list), 'minmaxvalues_all_data_2012', 'next_close')
        ...
        # plt.plot(labels['next_close'])
        # plt.show()

    return errors, predicted_close, actual_values_forward, actual_list_buffer, backward_date_time,forward_date_time

def predict_arima_ann(date_time,count):

    errors, predictions_from_ann, actual_values_forward, actual_close,backward_date_time,forwared_date_time = predict(date_time,count)
...
#    actual_values_forward = pd.DataFrame({'actual_values_forward': actual_values_forward})
#    actual_values_forward.index = [10, 11, 12, 13, 14]

    final_results = pd.DataFrame({'final_results': final_results})
#    final_results.index = [10, 11, 12, 13, 14]


    predictions_from_ann = pd.DataFrame(predictions_from_ann)
    ...
    plt.plot(actual_values_forward, color='orange')
